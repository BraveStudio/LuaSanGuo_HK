...
--播放气泡动画
function Functions.playBubbleAction(target,text1,text2)
    if #target:getChildren() < 1 then 
        local node = cc.CSLoader:createNode("tyj/csb/common/bubble.csb")
        node:setPosition(cc.p(-50,100))
        local bubblePanel = node:getChildByName("bubblePanel")
       bubblePanel:getChildByName("bubble3"):getChildByName("line1"):setString(text1)
       bubblePanel:getChildByName("bubble3"):getChildByName("line2"):setString(text2)
        target:addChild(node)
        for i=1,3 do
            bubblePanel:getChildByName("bubble" .. tostring(i)):setVisible(false)
        end
        local actionList = {}
        for i=1,3 do
            UIActionTool:createCallFuncAction({actionList = actionList,handler = function()
                local bubbleNode = bubblePanel:getChildByName("bubble" .. tostring(i))
                bubbleNode:setVisible(true)
                local originalScaleX = bubbleNode:getScaleX()
                local originalScaleY = bubbleNode:getScaleY()
                UIActionTool:playSacleActionWithEase({node = bubbleNode,
                    startRate= {x=0,y=0},
                    endRate = {x = originalScaleX, y = originalScaleY},
                    easeType = EaseActionType.EaseElasticOut,
                    dur = 0.5
                    })
            end})
            UIActionTool:createDelayTimeAction({actionList = actionList,delayTime = 0.4 + 0.1*i}) 
        end
        UIActionTool:createDelayTimeAction({actionList = actionList,delayTime = 1})
        UIActionTool:createCallFuncAction({actionList = actionList,handler = function()
             local bubbleNode = bubblePanel:getChildByName("bubble3")
             local blinkAction = {}
             local scaleAction = UIActionTool:createJumpByAction({pos ={x = 0,y = 0}, height = 10, jumps = 10,dur = 0.2})
             UIActionTool:createEaseElasticOutAction({actionList = blinkAction, actionElement = scaleAction})
             -- local reverseAction = UIActionTool:createScaleToAction({sx = 1,sy = 1, dur = 0.1})
             -- UIActionTool:createEaseElasticOutAction({actionList = blinkAction, actionElement = reverseAction})
             Functions.playSequenceAction(bubbleNode,blinkAction,5,0)
        end})
        UIActionTool:createDelayTimeAction({actionList = actionList,delayTime = 30})
        UIActionTool:createCallFuncAction({actionList = actionList,handler = function()
            target:removeAllChildren()
        end})
        Functions.playSequenceAction(node,actionList,0,1)
    end
end
...
--播放openChildView动作，放到 getPopAction()方法中
function Functions.playScaleOpenChildViewAction(view,handler)
    UIActionTool:playSacleActionWithEase({node = view,startRate = {x = 0, y = 0},
        endRate = {x = 1,y = 1},easeType = EaseActionType.EaseBackOut,dur = 0.3,
    handler = handler})
end
--播放滑动到屏幕边缘，放到 getPopAction()方法中
function Functions.playSildeToEdgeAction(view,dir,handler)
    local fadeInAction = UIActionTool:createFadeOutAction({dur = 0.4})
    local easeRateAction = UIActionTool:createEaseRateActionAction({actionElement = fadeInAction,rate = 10})
    Functions.playSequenceAction(view:getController().blackColorLayers_t[view], {easeRateAction}, 0,1)
    local startPos,endPos
    if dir == MoveDirection.LEFT then
        endPos = cc.p(display.width,display.cy)
    elseif  dir == MoveDirection.RIGHT then
        endPos = cc.p(0,display.cy)
    elseif  dir == MoveDirection.UP then
        endPos = cc.p(display.cx,display.height + display.cy)
    elseif  dir == MoveDirection.DOWN then
        endPos = cc.p(display.cx,display.height)
    end
    startPos = display.center
    UIActionTool:playMoveActionWithEase({node = view,startPos = startPos,
        endPos = endPos,easeType = EaseActionType.EaseExponentialIn,dur = 0.4,
    handler = handler})
end
--播放滑动到屏幕中心动作，放到 getPopAction()方法中
function Functions.playSildeToCenterAction(view,dir,handler)
    
    local startPos,endPos
    if dir == MoveDirection.LEFT then
        startPos = cc.p(display.width,display.height + display.cy)
    elseif  dir == MoveDirection.RIGHT then
        startPos = cc.p(0,display.cy)
    elseif  dir == MoveDirection.UP then
        startPos = cc.p(display.cx,0)
    elseif  dir == MoveDirection.DOWN then
        startPos = cc.p(display.cx,display.height)
    end
    endPos = display.center
    UIActionTool:playMoveActionWithEase({node = view,startPos = startPos,
        endPos = endPos,easeType = EaseActionType.EaseExponentialOut,dur = 0.4,
    handler = handler})
end
--播放closeChildView动作
function Functions.playCloseChildViewAction(view,handler)
    local fadeInAction = UIActionTool:createFadeOutAction({dur = 0.3})
    local easeRateAction = UIActionTool:createEaseRateActionAction({actionElement = fadeInAction,rate = 10})
    Functions.playSequenceAction(view:getController().blackColorLayers_t[view], {easeRateAction}, 0,1)
    UIActionTool:playSacleActionWithEase({node = view,startRate = {x = 1, y = 1},
        endRate = {x = 0,y = 0},easeType = EaseActionType.EaseExponentialIn,dur = 0.3
        ,handler = handler})
end
--ui节点顺序播放一个动作序列
--@param : target 执行动画的ui widget 
...
--@param : delayTime 循环间隔时间 Number 
--@param : isHide 间隔时间中，是否隐藏动画节点 bool
--@param : count 动画列表播放次数 0 为永久播放
function Functions.playSpawnAction(target, animaNameTable, delayTime,count,isHide,handler)
    if delayTime == nil then delayTime = 0 end
    if count == nil then count = 0 end
    if isHide == nil then isHide = false end

    local spawnAction = {}
    --    spawnAction[#spawnAction+1] = cc.CallFunc:create(function() target:show() end)
    ...
    for i = 1, #animaNameTable do 
        ...
        spawnAction[#spawnAction+1] = repeatAction
    end

    spawnAction[#spawnAction+1]= cc.CallFunc:create(function()
        if isHide then
            target:hide()
        end
        if handler ~= nil then 
    ...
    if delayTime > 0 then 
        spawnAction[#spawnAction+1] = cc.DelayTime:create(delayTime)
    end
    spawnAction[#spawnAction+1] = NULL
    local play = cc.Spawn:create(unpack(spawnAction))
    target:setVisible(true)
    if count > 0 then
        transition.execute(target,cc.Repeat:create(play,count)) 
    else
...
function Functions.showHeroHead(target,heroData,isSelected)
    ...
    local levelLabel = target:getChildByName("levelPanel"):getChildByName("level")
    Functions.initHeroName(nameLabel,heroData.m_id,bigClass)
    levelLabel:setString(tostring(heroData.m_level or 1))
...
--根据一个英雄的ID显示一个英雄头像
--heroData(m_id,m_class)
function Functions.showpreviewHeroHead(target,heroData)
    if heroData.m_id == nil then
	   return false
    end
    if heroData.m_class == nil then
        heroData.m_class = 1
    end
...
function Functions.initPokedexHeadWidget(target,id,m_state)
    ...
    local HeadImg = string.gsub (heroHeadImg, ".png", "_h.png")
    Functions.loadImageWithWidget(target,HeadImg)
end
--武将及选将中英雄卡牌显示相关函数end------------------------
...
function Functions.initItemComOfId(itemView, id)
    ...
    elseif id < 0 then
        ...
        elseif id == -6 then
            img = "property_soulCrystal.png"
        elseif id == -7 then
            img = "commonUI/res/image/jifen.png"
        elseif id == -10 then
            img = "commonUI/res/image/gongxian.png"
        else
...
function Functions.setAdbrixTag(tagType,tagName,secondParameter)
    ...
    if G_IsUseSDK and G_SDKType ~= 6 and G_SDKType ~= 7 then
...
--获得将魂图标
--
function Functions.getHeroSoulNodeOfId(target,id,handler)
    local image = ConfigHandler:getHeroSoulImageOfId(id)
    local imageView = target:getChildByName("heroSoulPanel"):getChildByName("heroSoul")
    imageView:setVisible(true)
    imageView:setScale(1.2)
    if handler ~= nil then 
        imageView:onTouch(Functions.createClickListener(handler, ""))
    end
    Functions.loadImageWithWidget(imageView,image)    
    local heroSoulType = ConfigHandler:getHeroSoulTypeOfId(id)
    local typeNode = target:getChildByName("heroSoulPanel"):getChildByName("typeNode")
    Functions.loadImageWithWidget(typeNode,"tyj/dynamicUI_res/heroSoulType_" .. tostring(heroSoulType) .. ".png")  
    typeNode:setVisible(true)  
    local kuangTable = {"baikuang.png","lukuang.png","lankuang.png","zikuang.png","hongkuang.png","chengkuang.png"}
    local kuangView = target:getChildByName("heroSoulPanel"):getChildByName("choose")
    kuangView:setVisible(true)
    local quality = ConfigHandler:getHeroSoulQualityOfId(id)
    Functions.loadImageWithWidget(kuangView,kuangTable[quality])
end
function Functions.getHeroSoulNodeOfMark(target,mark,handler)
    if mark > 0 then 
        local heroSoulInf = HeroSoulData:getHeroSoulInf(mark)
        Functions.getHeroSoulNodeOfId(target,heroSoulInf.id,handler)
        local levelView = target:getChildByName("heroSoulPanel"):getChildByName("level")
        if heroSoulInf.level > 0 then 
            levelView:setVisible(true)
            levelView:setString("Lv." .. tostring(heroSoulInf.level))
        else
            levelView:setVisible(false)
        end
        local apparelFlagView = target:getChildByName("heroSoulPanel"):getChildByName("status")
        if heroSoulInf.atkFormFlag > 0 then             
            apparelFlagView:setVisible(true)
            local resTable= {"tyj/uiFonts_res/bubing.png","tyj/uiFonts_res/qibing.png","tyj/uiFonts_res/gongbing.png"}
            Functions.loadImageWithWidget(apparelFlagView,resTable[heroSoulInf.atkFormFlag])
        else
            apparelFlagView:setVisible(false)
        end
    else
        Functions.cleanHeroSoulNode(target)
    end
end
--设置将魂选中状态
function Functions.setHeroSoulSelected(target,isSelected)
    local selectView = target:getChildByName("heroSoulPanel"):getChildByName("selected")
    if isSelected == nil then isSelected = false end
    if isSelected then
...
function Functions.cleanHeroSoulNodeWithAdd(target,handler)
    local addImg = "commonUI/res/icons/adddd.png"
    local heroSoulPanel = target:getChildByName("heroSoulPanel")
    --icon
    local heroSoulView = heroSoulPanel:getChildByName("heroSoul")
    heroSoulView:setVisible(true)
    
    heroSoulView:onTouch(Functions.createClickListener(handler, ""))
    Functions.loadImageWithWidget(heroSoulView, addImg)
    heroSoulView:setScale(1)
    local selectView = heroSoulPanel:getChildByName("selected")
    selectView:setVisible(false)
    local status = heroSoulPanel:getChildByName("status")
    status:setVisible(false)
    local level = heroSoulPanel:getChildByName("level")
    level:setVisible(false)
    local typeNode = heroSoulPanel:getChildByName("typeNode")
    typeNode:setVisible(false)
    local kuangView = heroSoulPanel:getChildByName("choose")
    kuangView:setVisible(false)
end
--清空将魂显示
function Functions.cleanHeroSoulNode(target,isOpen)
    local heroSoulPanel = target:getChildByName("heroSoulPanel")
    --icon
    local heroSoulView = heroSoulPanel:getChildByName("heroSoul")
    heroSoulView:setVisible(false)
    local selectView = heroSoulPanel:getChildByName("selected")
    selectView:setVisible(false)
    local status = heroSoulPanel:getChildByName("status")
    status:setVisible(false)
    ...
    local kuangView = heroSoulPanel:getChildByName("choose")
    kuangView:setVisible(false)
    local lock = heroSoulPanel:getChildByName("lock")
    if isOpen ~= nil then 
        lock:setVisible(isOpen)
    end
end
...
function Functions.displayHeroSoulInf(target,data,showType,handler)
    local heroSoulView = target:getChildByName("heroSoul")
    Functions.cleanHeroSoulNode(heroSoulView)
    ...
    local heroSoulId = 0 
    local heroSoulLevel = 1
    if showType == 4 then
        Functions.getHeroSoulNodeOfId(heroSoulView,data.heroSoulInf.id)   
        heroSoulId = data.heroSoulInf.id
    else
        Functions.getHeroSoulNodeOfMark(heroSoulView,data.mark)
        local heroSoulInf = HeroSoulData:getHeroSoulInf(data.mark)
        heroSoulId = heroSoulInf.id
        heroSoulLevel = heroSoulInf.level
    end

    local name = target:getChildByName("name")
    name:setString(ConfigHandler:getHeroSoulNameOfId(heroSoulId))
    
    attr:setString(attrTypeTable[ConfigHandler:getHeroSoulTypeOfId(heroSoulId)])

    local buff = target:getChildByName("buff")
    local buffValue = Functions.heroSoulAddAttrHandler(heroSoulId, heroSoulLevel)
    if ConfigHandler:getHeroSoulAtrrAddTypeOfId(heroSoulId) == 2 then
        buff:setString("+" .. tostring(buffValue*100) .. "%") 
    else
        buff:setString("+" .. tostring(buffValue*100))
    end

    heroType = target:getChildByName("type")
    heroType:setString(LanguageConfig["ui_heroSoul_" .. tostring(ConfigHandler:getHeroSoulTypeOfId(heroSoulId))])

    local inf = target:getChildByName("inf")
    inf:setString(ConfigHandler:getHeroSoulDescribeOfId(heroSoulId))

    local price = target:getChildByName("price")
    local quality = ConfigHandler:getHeroSoulQualityOfId(heroSoulId)

    price:setString(tostring(g_jhcfg.sellPrice[quality][heroSoulLevel]))   
    ...
    for i=1, num do           
        ...
        if isShowInf ~= nil then 
            local onDisNodeClick = function()
                PromptManager:openInfPrompt({type = prizeData[i][2],id = prizeData[i][1],target = disNode},false)
            end
            local touchObj = nil 
...
-- end
--获取上阵英雄总战斗力
--@param: heros format {embattleType, mainHeroMark , viceHeroMarks, partHeroMarks,embattleHeroMarks}
function Functions.getFinalFightAttrs(inf)
    ...
    if inf.mainHeroMark > 0 then
        ...
        mainHeroFightValue = Functions.getHeroFightAttrs({heroMark=inf.mainHeroMark,
            partHeroMarks = inf.partHeroMarks, equipMarks = equipMarks,
            embattleHeroMarks = inf.embattleHeroMarks})
    end
    if inf.viceHeroMarks[1] > 0 then
        local equipMarks = EquipmentData:getEquipMarks(inf.embattleType,2)
        viceHero1FightValue = Functions.getHeroFightAttrs({heroMark=inf.viceHeroMarks[1],partHeroMarks = inf.partHeroMarks, equipMarks = equipMarks,embattleHeroMarks = inf.embattleHeroMarks})
    end
    if inf.viceHeroMarks[2] > 0 then
        local equipMarks = EquipmentData:getEquipMarks(inf.embattleType,3)
        viceHero2FightValue = Functions.getHeroFightAttrs({heroMark=inf.viceHeroMarks[2],partHeroMarks = inf.partHeroMarks, equipMarks = equipMarks,embattleHeroMarks = inf.embattleHeroMarks})
    end
...
--获取某个上阵英雄的最终属性
--@inf = {heroMark,partHeroMarks,equipMarks,embattleHeroMarks}
function Functions.getHeroFightAttrs(inf)
    ...
    local heroInfos = {}
    for k,v in pairs(inf.embattleHeroMarks) do 
        if v > 0 then 
            heroInfos[#heroInfos + 1] = HeroCardData:packageHeroAttr(v)
        end
    end
    return cs_GetCardFightValue({heroInfo = HeroCardData:packageHeroAttr(inf.heroMark),
        partHeros = partHerosInf,
        equipInfos = equipPackageAttr,
        heroInfos = heroInfos})
end
--绑定checkBox监听
...
-- @descri 获取士兵的类型
function Functions.getSoldierhTypeOfId(id)
    return math.floor(tonumber(id)/1000)
end

function Functions.heroSoulAddAttrHandler(id, level, attrType)
    local addBaseValue = ConfigHandler:getHeroSoulBaseOfId(id)
    local levelUpValue = ConfigHandler:getHeroSoulAddvalueOfId(id)
    local  value = addBaseValue + (level - 1)*levelUpValue
    return value
end

function Functions.getSoldierhEquipAttr(equipments, jhBook, oldValue, soldierType, attrType)
    assert(equipments and type(equipments) == "table" and oldValue and soldierType and attrType, "input param is error!!!")
    
    local addValue = 0
    local baseAddAtrrHandler = function(id, level, oldValue, attrType)
        local value = 0
        local attrAddType = ConfigHandler:getHeroSoulAtrrAddTypeOfId(id)
        if ConfigHandler:getHeroSoulTypeOfId(id) == attrType then
            if attrAddType == SoldierAtrrAddType.ValueAdd then
                value = Functions.heroSoulAddAttrHandler(id,level)
            elseif attrAddType == SoldierAtrrAddType.PercentAdd then
                local percentValue = Functions.heroSoulAddAttrHandler(id, level)
                value = oldValue * percentValue
            else
                assert(false, "士兵将魂属性改变类型配置错误")
            end
        end
    ...
    local qhAddAttrHandler = function(id, oldValue, attrType, soldierType)
        local value = 0
        local qhSoldierType = ConfigHandler:getHeroSoulKindSoldierOfId(id)

        if soldierType == qhSoldierType then
            if ConfigHandler:getHeroSoulKindTypeOfId(id) == attrType then
                value = ConfigHandler:getHeroSoulKindRateOfId(id)*oldValue
            end
        end
    ...
    for k, v in pairs(equipments) do
        ...
        local id = v.m_id
        local level = v.m_level
        if id > 0 and level > 0 then
            --计算基础部分增加
            addValue = addValue + baseAddAtrrHandler(id, level, oldValue, attrType)
        end
    end

    for k, v in pairs(jhBook) do
        local id = v
        --计算亲和部分增加
        addValue = addValue + qhAddAttrHandler(id, oldValue, attrType, soldierType)
    end
...
-- @descri 获取士兵的血量
-- @param id: 士兵id lv: 士兵等级 equipments: 士兵装备集合
-- @return 士兵血量
function Functions:getSoldierhHp( id, lv, equipments, jhBook)
    ...
    if equipments and #equipments > 0 then
        local soldierType = Functions.getSoldierhTypeOfId(id)
        hp = hp + Functions.getSoldierhEquipAttr(equipments[soldierType], jhBook, hp, soldierType, CombatAttrType.hp )
    end
...
-- @return 士兵攻击
-- @descri 获取士兵的攻击
function Functions:getSoldierhAtk( id, lv, equipments, jhBook)
    local SoldiersData = ConfigHandler:getSoldierInfosOfId(id)
    local atk = 0  --初始化小兵攻击
    ...
    if equipments and #equipments > 0 then
        local soldierType = Functions.getSoldierhTypeOfId(id)
        atk = atk + Functions.getSoldierhEquipAttr(equipments[soldierType], jhBook, atk, soldierType, CombatAttrType.attack )
    end
    return atk
end
...
-- @return 士兵法术
-- @descri 获取士兵的法术
function Functions:getSoldierhFas( id, lv, equipments, jhBook)
    local SoldiersData = ConfigHandler:getSoldierInfosOfId(id)
    local fas = 0  --初始化小兵法术
    fas = SoldiersData.origFas + SoldiersData.growFas*( lv - 1 )
    if equipments and #equipments > 0 then
        local soldierType = Functions.getSoldierhTypeOfId(id)
        fas = fas + Functions.getSoldierhEquipAttr(equipments[soldierType], jhBook, fas, soldierType, CombatAttrType.fas )
    end
    return fas
end
...
-- @return 士兵法防
-- @descri 获取士兵的法防
function Functions:getSoldierhFaf( id, lv, equipments, jhBook)
    local SoldiersData = ConfigHandler:getSoldierInfosOfId(id)
    local faf = 0  --初始化小兵法防
    ...
    if equipments and #equipments > 0 then
        local soldierType = Functions.getSoldierhTypeOfId(id)
        faf = faf + Functions.getSoldierhEquipAttr(equipments[soldierType], jhBook, faf, soldierType, CombatAttrType.faf )
    end
...
-- @return 士兵暴击
-- @descri 获取士兵的暴击
function Functions:getSoldierhBaoj( id, equipments, jhBook)
    local SoldiersData = ConfigHandler:getSoldierInfosOfId(id)
    local baoj = g_baseBaoj  --初始化小兵法防

    if equipments and #equipments > 0 then
        local soldierType = Functions.getSoldierhTypeOfId(id)
        baoj = baoj + Functions.getSoldierhEquipAttr(equipments[soldierType], jhBook, baoj, soldierType, CombatAttrType.baoj )
    end
...
-- @return 士兵暴击
-- @descri 获取士兵的暴击
function Functions:getSoldierhBaos( id, equipments, jhBook)
    local SoldiersData = ConfigHandler:getSoldierInfosOfId(id)
    local baos = g_baseBaos  --初始化小兵法防

    if equipments and #equipments > 0 then
        local soldierType = Functions.getSoldierhTypeOfId(id)
        baos = baos + Functions.getSoldierhEquipAttr(equipments[soldierType], jhBook, baos, soldierType, CombatAttrType.baos )
    end
...
function Functions.openNetWorkTip(errorCode, msg)
    ...
    elseif errorCode == NetWork.WebErrCode.User_Login_Invalid then
         PromptManager:openTipPrompt(LanguageConfig.User_Login_Invalid)
...
--生成一个人物头像node
function Functions.createHead(id)
    local node = cc.CSLoader:createNode("commonUI/csb/heroHead2.csb")

    --显示武将头像信息
    node:getChildByName("model"):setTouchEnabled(false)
    local heroData = { m_id = id}
    Functions.showpreviewHeroHead(node:getChildByName("model"),heroData)
    return node
end
...
function Functions.jumpFbSeleckOfData(param)
    ...
    BiographyData.isOpenAwardPanel = param.isOpenAward
...
function GetRoundValue(min, max, value)
