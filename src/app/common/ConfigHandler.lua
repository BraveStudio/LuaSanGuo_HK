...
elseif G_CurrentLanguage == "hk" then
    ConfigHandler.Language = "港澳台"
end
...
function ConfigHandler:init()
    ...
    self.itemPrice = TextReader.parserFile("data/itemprice.txt")
    self.itemPrice = TextReader.parserFile("data/itemprice.txt")
    self.squadInfos = TextReader.parserFile("data/zhengrong.txt",true)
end

--获取推荐阵容配置
function ConfigHandler:getSquadInfos(squadType,listener)
    local squadData = {}
    for k, v in pairs(self.squadInfos) do
        if v["阵容ID"] == squadType then 
            for y=1,3 do
                local str = string.format("队伍%d",y)
                if v[str] then
                    local data = Functions.strSplit(v[str], ",")
                    for m,g in pairs(data) do
                    	data[m] = tonumber(g)
                    end
                    squadData[#squadData + 1] = data
                end
            end
        end
    end
...
--获取将魂资源
--名字
function ConfigHandler:getHeroSoulNameOfId(id)
    return g_jhconfig[id].name
end
--类型
function ConfigHandler:getHeroSoulTypeOfId(id)
    return g_jhconfig[id].type
end
--品质
function ConfigHandler:getHeroSoulQualityOfId(id)
    return g_jhconfig[id].quality
end
--属性改变类型
function ConfigHandler:getHeroSoulAtrrAddTypeOfId(id)
    return g_jhconfig[id].atrrAddType
end
--基础属性
function ConfigHandler:getHeroSoulBaseOfId(id)
    return g_jhconfig[id].base
end
--增长值
function ConfigHandler:getHeroSoulAddvalueOfId(id)
    return g_jhconfig[id].addvalue
end
--亲和类型
function ConfigHandler:getHeroSoulKindTypeOfId(id)
    return g_jhconfig[id].kindType
end
--根据士兵类型获取亲和将魂id
function ConfigHandler:getKindHeroSoul(sodierType)
    local kindHeroSoul = {}
    for k,v in pairs(g_jhconfig) do 
        if v.kindSoldier == sodierType then
            kindHeroSoul[#kindHeroSoul+1] = v.id
        end
    end
    return kindHeroSoul
end
--亲和百分比
function ConfigHandler:getHeroSoulKindRateOfId(id)
    return g_jhconfig[id].kindRate
end
--亲和士兵
function ConfigHandler:getHeroSoulKindSoldierOfId(id)
    return g_jhconfig[id].kindSoldier
end
--资源图片
function ConfigHandler:getHeroSoulImageOfId(id)
    return g_jhconfig[id].imgID
end
--描述
function ConfigHandler:getHeroSoulDescribeOfId(id)
    return g_jhconfig[id].describe
end
--获取英雄缘分资源
...
function ConfigHandler:getOrigAttackOfId(id) --攻击
    return g_SoldierConfig[id].origAttack
end
function ConfigHandler:getOrigHPOfId(id) --生命
    return g_SoldierConfig[id].origHP
end
function ConfigHandler:getOrigFasOfId(id) --法术
    return g_SoldierConfig[id].origFas
end
function ConfigHandler:getOrigFafOfId(id) --法防
    return g_SoldierConfig[id].origFaf
end

function ConfigHandler:getHeroSoulOfId(id) --亲和将魂
    local script = " local heroSoul = " .. g_SoldierConfig[id].heroSoul .. "  return heroSoul"
    local heroSoul=assert(loadstring(script))()  
    return heroSoul
end

function ConfigHandler:getItemPrice(id,type) 
    local price = 0
    for k,v in pairs(self.itemPrice) do
    	if v["id"] == id and v["类型"] == type then
    		price = v["价格"]
    	end
    end
    return price
...
function ConfigHandler:getPspellsOfId(id, class)
    ...
    local pspells = {}
    for i=1, 10 do
        if info['passiveSpellId' .. i] then
            pspells[#pspells + 1] = info['passiveSpellId' .. i]
        else
            break
        end
    end
