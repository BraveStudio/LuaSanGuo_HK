...
function NetWork:setServerInfo(serverIp, serverId, port, name)
    if GetNetConfig() == NetWork.NetWorkType.Socket then
        ...
        SetServerIp(serverIp, serverId, port)
    else
        self.serverId = serverId
        self.serverName = name
        SetServerIp(serverIp, serverId, port)
...
function NetWork:loginUserServer(userName, userPassword, onSuccess)
    ...
    info["sdkType"] = G_SDKType
    if G_IsUseSDK  then 
        info["isUseSDK"] = "true"
    else
        info["isUseSDK"] = "false"
    end
    info["timestamp"] = GameState.storeAttr.timestamp_s
    info["sign"] = GameState.storeAttr.sign_s
