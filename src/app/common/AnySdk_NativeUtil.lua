...
function NativeUtil:startCheckExpansionFile(callBack)
    drmCallBack = callBack
    --添加update更新路径
    ...
    self:javaCallHanler({command = "checkExpansionFile"})
end
function NativeUtil._JniBackCall(msg)
    ...
    for k,v in pairs(msgTable) do
        ...
        elseif k == "API_Gateway" then 
            dump(v, "api_gateway")   
            GameState.storeAttr.curProductCode_s = v["productId"]
            GameState.storeAttr.paymentSeq_s = v["orderId"]
            GameState.storeAttr.curNonce_s = v["purchaseToken"]
            VipData:RequestVipPay(v["purchaseToken"],v["orderId"],v["productId"])
        ...
        elseif k == "openTips" then
            ...
            elseif v == "payFailed" then -- 提示玩家支付失败
                scheduler.performWithDelayGlobal(function ( )
