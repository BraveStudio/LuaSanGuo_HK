     ...
     self:setZhanGong(self.heroInf.progress)
      if HeroShiLianData.shiLianType == 0 then
          ...
          if self.heroInf.awarded == 1 then
              local heroHead = self._smCardBt_t:getChildByName("heroHead")
              heroHead:setVisible(true)
              -- Functions.getHeroCrad(heroCard,{id = self.heroInf.showheroid,class = 1})
              local heroHeadImg = ConfigHandler:getHeroHeadImageOfId(self.heroInf.showheroid) 
...
function HeroShiLianViewController:getHeroCrad()
   local handler = function(event)
          ...
          if HeroShiLianData.shiLianType == 0 then
              local heroHead = self._smCardBt_t:getChildByName("heroHead")
              heroHead:setVisible(true)
              local heroHeadImg = ConfigHandler:getHeroHeadImageOfId(event.gid) 
              Functions.loadImageWithWidget(heroHead, heroHeadImg)
          ...
          if HeroShiLianData.shiLianType == 0 then 
              ...
              local num = event.gnum
              Functions:addItemResources({id = event.gid,type = event.gtype,count = event.gnum,slot = event.gslot})   
              NoticeManager:openRewardTips(self, {type = NoticeManager.REWARD_CARD_FRAGMENT_TIPS,propData = event.gcount ,data = {{img = img, num = num}}}) 
          else
