...
function PropViewController:onDidLoadView()
	...
	self._heroSoulInfPanel_t = self.view_t.csbNode:getChildByName("main"):getChildByName("propListPanel"):getChildByName("propInfBg"):getChildByName("heroSoulInfPanel")
	self._bt_t = self.view_t.csbNode:getChildByName("main"):getChildByName("propListPanel"):getChildByName("propInfBg"):getChildByName("heroSoulInfPanel"):getChildByName("bt")
	self._upgradeBt_t = self.view_t.csbNode:getChildByName("main"):getChildByName("propListPanel"):getChildByName("propInfBg"):getChildByName("heroSoulInfPanel"):getChildByName("upgradeBt")
	...
	self._heroSoulContainer_t = self.view_t.csbNode:getChildByName("main"):getChildByName("propListPanel"):getChildByName("heroSoulContainer")
	...
	self._tils_t = self.view_t.csbNode:getChildByName("main"):getChildByName("propListPanel"):getChildByName("propInfBg"):getChildByName("heroSoulInfPanel"):getChildByName("tils")
	self._tils_t:setString(LanguageConfig.ui_equip_13)
...
function PropViewController:onBackbtClick()
    ...
    if self.onkeyCount ~= 0 then 
        ...
        self:setTableTouchable(true)
        if self.currentSystem == 2 then 
        ...
        elseif self.currentSystem == 3 then
            local heroSoulData = HeroSoulData.heroSoulInf
            local sortHeroSoulData =  HeroSoulData:sortHeroSoul(heroSoulData)
            self:setOneKeySellHeroSoul(sortHeroSoulData,nil)
            self:displayHeroSoulList(sortHeroSoulData,1)
        end
...
function PropViewController:onQuedbtClick()
    ...
    local handler = function (event)
        ...
        elseif self.currentSystem == 3 then
            self:setOneKeySellHeroSoul( HeroSoulData.heroSoulInf,nil)
            for k,v in pairs(self.willSellHeroSoulMarks) do 
                HeroSoulData:miuHeroSoul( v )
            end
            self.willSellHeroSoulMarks = {}
            self:displayHeroSoulList(HeroSoulData.heroSoulInf,1)
            PromptManager:openTipPrompt(LanguageConfig.language_prop_18) 
        end
    end
    ...
    elseif self.currentSystem == 3 then
         if #self.willSellHeroSoulMarks > 0 then 
            HeroSoulData:requestSellHeroSoul(self.willSellHeroSoulMarks,handler)
        end
    end
end
...
function PropViewController:onOnekeybtClick()
    ...
    if self.currentSystem == 2 then 
        self:oneKeySellEquip()
    elseif self.currentSystem == 3 then
        self:oneKeySellHeroSoul()
    end
end
...
function PropViewController:oneKeySellHeroSoul()
    self._upgradeBt_t:setVisible(false)
    self._bt_t:setPositionX(110)
    self._quedBt_t:setVisible(true)
    self:setTableTouchable(false)
    local heroSoul = HeroSoulData:getHeroSoulWithoutAppareled(HeroSoulData.heroSoulInf)
    local sortheroSoul =  HeroSoulData:sortHeroSoul(heroSoul)
...
--        self:setOneKeySellEquip(false)
        ...
        self:setOneKeySellHeroSoul(sortheroSoul,false)
        self:displayHeroSoulList(sortheroSoul,2)
    elseif self.onkeyCount == 1 then --键选中
        ...
        local heroSoulData = HeroSoulData:getColorNumUpEquip(4)
        self:setOneKeySellHeroSoul(heroSoulData,true) 
        GameEventCenter:dispatchEvent({ name = "HEROSOUL_ONEKEY_SELECTE_EVENT"}) 
        if #self.willSellHeroSoulMarks > 0 then 
            self._quedBt_t:setVisible(true)
        else
...
function PropViewController:onDisplayView()
    ...
    self.willSellHeroSoulMarks = {} --将要被卖出的将魂
    ...
    local selectTableListener = function(target)
        ...
        elseif target == "tb3" then
            if self.onkeyCount == 0 then 
                self._bt_t:setVisible(true)
                local heroSoulData = HeroSoulData.heroSoulInf
                local sorHeroSoulData =  HeroSoulData:sortHeroSoul(heroSoulData)
                self:displayHeroSoulList(sorHeroSoulData,1)
                self.currentSystem = 3 
            end                 
        end
    end
...
function PropViewController:setTableTouchable(isEnable)
    for i=1,3 do 
        self._table_t:getChildByName("tb" .. tostring(i)):setTouchEnabled(isEnable)
    end
end
function PropViewController:setOneKeySellHeroSoul(heroSoulData,selecetFlag)
    for k,v in pairs(heroSoulData) do 
        ...
        -- local quality = ConfigHandler:getHeroSoulQualityOfId(v.id)
        if selecetFlag == true then 
            self:addWillSellHeroSoulMark(v.mark)
            self:updateMoneyOfHeroSoul() 
        elseif selecetFlag == false then 
            table.removebyvalue(self.willSellHeroSoulMarks,v.mark)
            self:updateMoneyOfHeroSoul()
        end
    end
end
...
function PropViewController:addWillSellHeroSoulMark(mark)
    for k,v in pairs(self.willSellHeroSoulMarks) do 
        if v == mark then 
            return
        end
    end
    self.willSellHeroSoulMarks[#self.willSellHeroSoulMarks+1] = mark
end
...
function PropViewController:updateMoneyOfHeroSoul( )
    self.huiShouMoney = 0 
    for k,v in pairs(self.willSellHeroSoulMarks) do 
        self.huiShouMoney = self.huiShouMoney + HeroSoulData:getHeroSoulPrice(v)
    end
...
--显示将魂列表
function PropViewController:displayHeroSoulList(showData,showType)
    self:setContainerVisible("heroSoul")
    ...
    local listHandler = function(index, widget, model, data )
        ...
        if index == 1  and selectedFlag == 0 then
            selectedFlag = index
            if showType == 1 then 
                Functions.setHeroSoulSelected(widget,true)

            else
                Functions.setHeroSoulSelected(widget,false) 
            end
            self._heroSoulInfPanel_t:setVisible(true)
            Functions.displayHeroSoulInf(self._heroSoulInfPanel_t,data)

            self:bindSellHeroSoulBtHandler(widget,data)
            local qiangHuaBtHandler = function( )
                self:openChildView("app.ui.popViews.HeroSoulUpgradePopView",{isRemove = false,name = "HeroSoulUpgradePopView",data = {mark = data.mark,
                    handler = function()
                        local heroSoulData = HeroSoulData.heroSoulInf
                        local sortHeroSoulData =  HeroSoulData:sortHeroSoul(heroSoulData)
                        self:displayHeroSoulList(sortHeroSoulData,showType)
                end}})
            end
            self._upgradeBt_t:onTouch(Functions.createClickListener(qiangHuaBtHandler,"zoom"))
            if showType == 1 then 
                local level = HeroSoulData:getHeroSoulInf(data.mark).level
                if level < g_jhcfg.maxLevel then 
                    self._upgradeBt_t:setVisible(true)
                    self._bt_t:setPositionX(58)
                else
                    self._upgradeBt_t:setVisible(false)
                    self._bt_t:setPositionX(110)
        ...
        Functions.bindEventListener(widget,GameEventCenter,"HEROSOUL_CANCAL_SELECTED_EVENT",function()  -- 点中装备
            if selectedFlag == widget.index  then             
                Functions.setHeroSoulSelected(widget,false) 
            end
        end)
        Functions.bindEventListener(widget,GameEventCenter,"HEROSOUL_ONEKEY_SELECTE_EVENT",function() --一键选择将魂出售
            -- local quality = ConfigHandler:getHeroSoulQualityOfId(data.id)
            if data.selecetFlag ~= nil then 
                widget:getChildByName("heroSoulPanel"):getChildByName("CheckBox_Mark"):setSelectedState(data.selecetFlag) 
            end
        end)
        local clickHandler = function()
            if selectedFlag ~= index then 
                GameEventCenter:dispatchEvent({ name = "HEROSOUL_CANCAL_SELECTED_EVENT"})
            end
            if showType == 1 then 
                Functions.setHeroSoulSelected(widget,true)
            else
                ...
                if not checkBox:isSelected() then 
                    ...
                    self:addWillSellHeroSoulMark(data.mark)
                    self:updateMoneyOfHeroSoul()
                else
                    data.selecetFlag = false
                    checkBox:setSelectedState(false)
                    table.removebyvalue(self.willSellHeroSoulMarks,data.mark)
                    self:updateMoneyOfHeroSoul()
                end
                if #self.willSellHeroSoulMarks > 0 then 
                    self._quedBt_t:setVisible(true)
    ...
    local cleanNodeHandler = function(target)
       ...
       Functions.setHeroSoulSelected(target,false) 
       target:getChildByName("heroSoulPanel"):getChildByName("CheckBox_Mark"):setVisible(false)
    end
    local romveNodeHandler = function(target)
       Functions.removeEventBeforeUiClean(target)
    end
    if #showData < 1 or self:isHaveSellHeroSoul() then 
        self._heroSoulInfPanel_t:setVisible(false)
        self._oneKeyBt_t:setVisible(false)
    end
    Functions.bindTableViewWithData(self._heroSoulContainer_t,{firstData = showData,
...
--设置显示的界面
function PropViewController:setContainerVisible(displayObject)
    if displayObject == "heroSoul" then
        self._btPanel_t:setVisible(true)
        self._oneKeyBt_t:setVisible(true)
        self._quedBt_t:setVisible(false)

        self._equipContainer_t:setVisible(false)
        self._heroSoulContainer_t:setVisible(true)
        ...
        self._heroSoulInfPanel_t:setVisible(true)
    elseif displayObject == "equip" then 
        self._btPanel_t:setVisible(true)
        self._oneKeyBt_t:setVisible(true)
        self._quedBt_t:setVisible(false)

        self._equipContainer_t:setVisible(true)
        self._heroSoulContainer_t:setVisible(false)
        self._propContainer_t:setVisible(false)
        ...
        self._heroSoulInfPanel_t:setVisible(false)
    elseif displayObject == "prop" then
...
function PropViewController:displayEquipList(showData,showType)
    self:setContainerVisible("equip")
...
function PropViewController:isHaveSellHeroSoul()
    local heroSoul = HeroSoulData:getHeroSoulWithoutAppareled(HeroSoulData.heroSoulInf)
    if #heroSoul < 1 then
        return true
    else
...
--绑定将魂出售按钮监听
function PropViewController:bindSellHeroSoulBtHandler(target,data)
    local sellBt = self._heroSoulInfPanel_t:getChildByName("bt")
    local clickHandler = function()
        if HeroSoulData:isAppareled(data.mark) then
            PromptManager:openTipPrompt(LanguageConfig.language_prop_16)
        elseif ConfigHandler:getHeroSoulQualityOfId( data.id) >= 3 or HeroSoulData:getHeroSoulLevel(data.mark) > 1 then
            local callBack = function()       
                self:sellHeroSoul( target,data.mark )
            end
            NoticeManager:openTips(self,{title = LanguageConfig.language_prop_17,handler = callBack})
        else
           self:sellHeroSoul(target,data.mark )
        end
    end
...
--出售将魂
function PropViewController:sellHeroSoul(target,mark)
    local handler = function (event)
        --出售成功获得铜钱
        PlayerData.eventAttr.m_money =  PlayerData.eventAttr.m_money + HeroSoulData:getHeroSoulPrice(mark)
        
        HeroSoulData:miuHeroSoul( mark )
        Functions.cleanHeroSoulNode(target)
        self._heroSoulInfPanel_t:setVisible(false)
        target:setTouchEnabled(false)
        self:displayHeroSoulList(HeroSoulData.heroSoulInf,1)
    ...
    HeroSoulData:requestSellHeroSoul({mark},handler)
end
--出售装备
...
function PropViewController:displayPropList()
    ...
    local listHandler = function(index, widget, model, data)
        ...
        print("data.m_iddata.m_iddata.m_iddata.m_iddata.m_id", data.m_id)
...
function PropViewController:requestOpenBox(id)
    local handler = function(event)                
        ...
        elseif event.gtype == 2 then--抽装备/将魂
        ...
        elseif event.gtype == 10 then--抽装备/将魂
            HeroSoulData:addheroSoul({id = event.gid,mark = event.gslot,
                level = 1,
                atkFormFlag = 0,
                exp = 0})
            if PropData:getPropNumOfId(id) >= 1 then 
                NoticeManager:openRewardTips(self, {type = NoticeManager.REWARD_HERO_SOUL_TIPS,data = {heroSoulMark = event.gslot,propData = id,againHandler = handler(self,self.requestOpenBox)}})   
            else
                NoticeManager:openRewardTips(self, {type = NoticeManager.REWARD_HERO_SOUL_TIPS,data = {heroSoulMark = event.gslot}})
            end   
