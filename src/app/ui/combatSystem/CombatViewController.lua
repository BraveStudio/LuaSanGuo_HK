...
function CombatViewController:onPausebtClick()
    ...
    if not self.isPause then
        NoticeManager:openTips(self, { title = LanguageConfig.ui_CombatView_Back_Title, handler = function()
            cc.Director:getInstance():resume()
            self.isPause = false
            self:forceQuitCombat()
        end, handler1 = function()
            cc.Director:getInstance():resume()
            self.isPause = false
        end, isNotPopAction = true })
...
function CombatViewController:onSpeed1btClick()
    ...
    if PlayerData.eventAttr.m_level >= g_csBaseCfg.addSpeedLevel then
        ...
        PromptManager:openTipPrompt(string.format(LanguageConfig.ui_CombatView_tip_3, g_csBaseCfg.addSpeedLevel))
    end
end
...
function CombatViewController:onGuideFunc()
    ...
    PromptManager:openDialoguePrompt(1008611, function (  )
        self:combatResLoad()
    end)
end
    ...
    Functions.tableSeqFunc(models_temp, function(model, endCallBack) --单模型加载回调函数
            ...
            elseif PlayerData.eventAttr.m_guideId == 8 then 
                    transition.execute(self.view_t, cc.DelayTime:create(1), {
                       onComplete = function()
                            self.isLogicPause = true
                            PromptManager:openNewGuide(self._speed1Bt_t, LanguageConfig.guide_4_14, function()
                               self.isLogicPause = false
                               CombatCenter:combatStart()
                               self:updateViewMoveAnima()
                            end)
