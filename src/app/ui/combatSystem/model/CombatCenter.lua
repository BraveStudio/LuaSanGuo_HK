...
function CombatCenter:getCombatHeroInfos(heroInfos)
    ...
    heroCombatInfos.soldierEquip = heroInfos.soldierEquip

    heroCombatInfos.jhBook = heroInfos.jhBook

    --添加小兵
    for i=1, GetRoundValue(0, CombatCenter.SoldiersNum, #heroInfos.Soldiers) do
...
function CombatCenter:getCombatBeforeHeroInfos(heroInfos)
    ...
    local getHeroInfos = function()
        local infos = {}

        infos[#infos+1] = heroInfos.MainHero[1]
        if heroInfos.ViceHeros then
    ...
    heroBeforeInfos.power = heroBeforeInfos.power + cs_GetCardFightValue({ heroInfo = heroInfos.MainHero[1],
                        partHeros = heroInfos.PartHero, equipInfos = equipInfos[1], heroInfos = getHeroInfos() })
    ...
    for k, v in pairs(heroInfos.ViceHeros) do
        ...
        heroBeforeInfos.power = heroBeforeInfos.power + cs_GetCardFightValue({ heroInfo = v,
                        partHeros = heroInfos.PartHero, equipInfos = equipInfos[k+1], heroInfos = getHeroInfos()})
    end
...
function CombatCenter:getSoldierCountOfCombat(combatType, isFrdly, playerLevel, mainHeroLevel, preSoldierLevel)
    local leadCount = 0
    ...
    elseif combatType == CombatCenter.CombatType.RB_PVE or 
        ...
        if isFrdly then
            leadCount = Functions.getSoldierCountOfLevel(playerLevel)
        else
            leadCount = Functions.getSoldierCountOfLevel(preSoldierLevel)
        end
    else
        if isFrdly then
            leadCount = Functions.getSoldierCountOfLevel(playerLevel)
        else
            leadCount = Functions.getSoldierCountOfLevel(mainHeroLevel)
        end
    end
...
function CombatCenter:initCombatOfInfo_(creatureInfos, isFrdly, combatType, heroList, combatTime)
    ...
    local soldierEquip = creatureInfos.soldierEquip
    local jhBook = creatureInfos.jhBook

    -- 获取英雄数据
    local heroInfos   = {}
    for k, v in pairs(peopleInfos) do
        if v.type == "hero" or v.type == "Commander1" or v.type == "Commander2" then
            heroInfos[#heroInfos + 1] = v
        end
    end
        ...
        if heroId > 0 then
            ...
            if peopleInfos[i].type == "hero" then
                ...
                if not peopleInfos[i].isDeath then
                    ...
                    creature = CreatureFactory:createHero(peopleInfos[i], partHeros, equipInfo, heroInfos, isFrdly,
                        CreatureModel.viewType.lord, self:createPos({ x = peopleInfos[i].x, y = peopleInfos[i].y }, 0, isFrdly), combatType, heroZxID, combatTime)

                    leadCount = self:getSoldierCountOfCombat(combatType, isFrdly, PlayerData.eventAttr.m_level, peopleInfos[i].level,
                        peopleInfos[i].solderNumLevel)
    ...
    --初始化战场小兵
    CombatCenter:initCombatSoldiers(peopleInfos, leadCount, combatType, isFrdly, heroZxID, m_lead_enemy, combatTime, soldierEquip, jhBook)
...
function CombatCenter:initCombatSoldiers(infos, leadCount, combatType, isFrdly, heroZxID, m_lead_enemy, combatTime, soldierEquip, jhBook)
    
    local creatures = nil
    if not isFrdly and combatType ~= CombatCenter.CombatType.RB_PVPHistoryData then
        m_lead_enemy = m_lead_enemy*2
    end

    for i=1, #infos do
        local soldierId = tonumber(infos[i].id)
        if soldierId > 0 then
            if infos[i].type == "soldier" then
                creatures = CreatureFactory:createSoldiers(tonumber(infos[i].id), infos[i].level, isFrdly,
                    { x = infos[i].x, y = infos[i].y }, leadCount, m_lead_enemy, handler(self, self.createPos), combatType, heroZxID, infos[i].gainValue, combatTime, soldierEquip, jhBook)
