    ...
    if self:checkXueZhanTime() then 
        if XueZhanData.xueZhanData.m_xzlyPass <= g_VipCgf.xueZhanLevels then
            ...
            if XueZhanData.xueZhanData.m_xzlyCount < g_VipCgf.xzlyAllCount[VipData.eventAttr.m_vipLevel] then
                if self:isPackageEnough(passType,XueZhanData.xueZhanData.m_xzlyPass,self.passFlag) then 
                    if self.passFlag then 
                        local callBack = function (data)
                            ...
                            for k,v in pairs(data.reward) do
                                ...
                                local hunjing  = 0
                                local prize = {}
                                for i,j in pairs(v.base) do 
                                    Functions:addItemResources({id = j[1],type = j[2],count = j[3]})
                                    if j[1] == -3 and j[2] == 4 then 
                                        coin = j[3]
                                    end
                                    if j[1] == -6 and j[2] == 4 then 
                                        hunjing = j[3]
                                    end
                                end
                                for m,n in pairs(v.extra) do 
                                    Functions:addItemResources({id = n[1],type = n[2],count = n[3]})
                                    prize[#prize+1] = n
                                end
                                 prizeTabel[#prizeTabel+1] = {coin = coin ,hunjing = hunjing,prize = prize}
                            end
                            self:openChildView("app.ui.popViews.SweepCompletePopView", { isRemove = false, data = { prizeTabel = prizeTabel,
                             handler =function( )
                                 XueZhanData:loadXueZhanData(handler(self,self.refrashInf))
                             end} })
                        end 
                        BiographyData:getSweepData(nil,callBack,passType)
                    else
                        GameCtlManager:push("app.ui.combatSystem.CombatViewController", { data = { combatType = CombatCenter.CombatType.RB_BloodyBattle,
                        majorHurdles = passType, littleLevels = XueZhanData.xueZhanData.m_xzlyPass ,isPassFlag = false} })     
                    end
...
--预判卡包及背包是否充足
function XueZhanViewController:isPackageEnough(passType,passLevle,passFlag)
    local extPrizeTable = {}
    if passFlag then 
        for i = 0, XueZhanData.xueZhanData.m_xzGetBuff - passLevle do 
            extPrizeTable[#extPrizeTable+1] = ConfigHandler:getXueZhanPassExtPrize(passType,passLevle + i)
            if (passLevle + i) % 5 == 0 then  
                break
            end
        end
    else
        extPrizeTable[#extPrizeTable+1] = ConfigHandler:getXueZhanPassExtPrize(passType,passLevle)
    end
    --合并到一个表中
    local mergePrize = {}
    for i = 1,#extPrizeTable do 
        for k,v in pairs(extPrizeTable[i]) do
            mergePrize[#mergePrize+1] = v
        end
    end 

    local mergePrizeCopy = clone(mergePrize)
    for k,v in pairs(mergePrize) do
        for i,j in pairs(mergePrizeCopy) do
            if v[2] == j[2] and v[1] == j[1] then
                v[3] = v[3] + j[3]
            end
        end
    end

    for k,v in pairs(mergePrizeCopy) do 
