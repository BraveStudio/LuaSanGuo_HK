...
--
function PromptManager:openBubblePrompt(text1,text2)
    if not self.bubblePanleView then
        self.bubblePanleView = CommonWidgets:getBubblePanel()
        self.bubblePanleView:getChildByName("line1"):setString(text1)
        self.bubblePanleView:getChildByName("line2"):setString(text2)
        self.bubblePanleView:setPosition(display.cx + 350,display.cy + 150)
        Functions.addCleanFuncWithNode(self.bubblePanleView, function()
            self.bubblePanleView = nil
        end)
        GameCtlManager:addCurBottomLayer(self.bubblePanleView)
    end
end
--弹出道具或英雄详细信息
...
function PromptManager:openInfPrompt(data,isShowCloseBt)
    if data.type == ItemType.HeroCard or data.type == ItemType.Prop or data.type == ItemType.CardFragment then
        if not self.tipPanel3View then
            ...
            self.tipPanel3View:onTouch(Functions.createClickListener(function( )
            ...
            elseif data.type == ItemType.Prop then
                ...
                local propNum = 0 
                if data.id < 0 then 
                    inf:setString(LanguageConfig["language_fu_" .. tostring(math.abs(data.id))])
                    if data.id == -2 then 
                        Functions.loadImageWithWidget(head, "property_gold.png")
                        propNum =  PlayerData.eventAttr.m_gold
                        name:setString(LanguageConfig.language_pay_2) 
                    elseif data.id == -3 then 
                        Functions.loadImageWithWidget(head, "property_money.png")
                        propNum =  PlayerData.eventAttr.m_money
                        name:setString(LanguageConfig.language_pay_6) 
                    elseif data.id == -5 then 
                        Functions.loadImageWithWidget(head, "property_soul.png")
                        propNum = PlayerData.eventAttr.m_soul
                        name:setString(LanguageConfig.language_pay_7) 
                    elseif data.id == -6 then
                        Functions.loadImageWithWidget(head, "property_soulCrystal.png")    
                        propNum = PlayerData.eventAttr.m_hunjing
                        name:setString(LanguageConfig.language_pay_8) 
                    elseif data.id == -7 then
                        Functions.loadImageWithWidget(head, "commonUI/res/image/jifen.png") 
                        head:setScale(1.8)  
                        propNum = TianTiData.eventAttr.m_tianTiScore
                        name:setString(LanguageConfig.language_pay_9) 
                    end
                else
                    ...
                    propNum = PropData:getPropNumOfId(data.id)
                end
                if propNum > 0 then
