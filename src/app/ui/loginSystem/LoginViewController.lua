...
function LoginViewController:openSelectServerPanel_()
    ...
    if G_DeviceType ~= 2 then
        NetWork:setServerInfo(self._server[index].ip, self._logicServerId, self._server[index].gateWayPort, self._server[index].name)
    else
        NetWork:setServerInfo(self._server[index].dn, self._logicServerId, self._server[index].gateWayPort, self._server[index].name)
    end
