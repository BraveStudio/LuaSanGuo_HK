...
--        self[state]:setScale(0.7)
        
        local scaleby = cc.ScaleTo:create(0.5, 1.1)
        local actionbyback = cc.ScaleTo:create(0.5, 0.7)
        local seq = cc.Sequence:create( scaleby,actionbyback)
        local repeatForever = cc.RepeatForever:create(seq)
        
        self[state]:stopAllActions()
        ...
        elseif v.state == 3 then
            Functions.loadImageWithSprite(self[state], "commonUI/res/icons/gongchengzhanzhong.png")
            self[state]:runAction(repeatForever)
        end
        ...
        if v.rewardState == 1 then
            ...
            self[Sprite_prize]:setPosition(cc.p(40, 8))
            self[Sprite_prize]:stopAllActions()
            local jumpby = cc.JumpTo:create(1, cc.p(40,8), 4, 20)
--            local easeSineIn = cc.JumpTo:create(1, cc.p(40, 8), 4, 20)

            local seq = cc.Sequence:create(jumpby, cc.DelayTime:create(2))
