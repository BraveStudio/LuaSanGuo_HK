...
function UnionViewController:onDidLoadView()
	...
	self._Text_level_shu_t = self.view_t.csbNode:getChildByName("main"):getChildByName("Panel_main"):getChildByName("Panel_union_main"):getChildByName("Text_level_shu")
	...
	self._Button_shop_t = self.view_t.csbNode:getChildByName("main"):getChildByName("Panel_main"):getChildByName("Panel_union_main"):getChildByName("Button_shop")
	self._Button_shop_t:onTouch(Functions.createClickListener(handler(self, self.onButton_shopClick), "zoom"))
...
--@auto code Button_shop btFunc end
...
function UnionViewController:onDisplayView()
    ...
    --监听公会等级
    local onLevel = function(event)
        --公告
        local data = UnionData:getUnionInfoData()
        Functions.initLabelOfString(self._Text_level_shu_t, data[1].eventAttr.guild_level)
    end
    Functions.bindEventListener(self.view_t, GameEventCenter, UnionData.UNION_LV_EVENT, onLevel)
        ...
        if event.reqtype == 1 then
    	     if event.ret == 1 then
                ...
                for k, v in pairs(data) do
                    ...
                    unionList.eventAttr.guild_level = v.guild_level
                ...
                local listHandler = function(index, widget, data, model)
                    ...
                    Functions.initTextColor(banModel:getChildByName("Text_levle_num"),ban:getChildByName("Text_levle_num"))
                    ...
                    local str = tostring(data.eventAttr.guild_level)..LanguageConfig.language_Union_45
                    ...
                    Functions.initLabelOfString( ban:getChildByName("Text_add_list_name"), data.eventAttr.s_name, ban:getChildByName("Text_add_list_level_num"), 
                        ...
                        ban:getChildByName("Text_levle_num"), str)
    ...
    local onUnionInfo = function(event)
        if  event.reqtype == 3 then
            if event.ret == 1 then
                ...
                self._Text_level_shu_t:setText(tostring(data.guild_level))
