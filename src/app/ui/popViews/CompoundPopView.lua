...
function CompoundPopView:showLevel()
    ...
    local listHandler = function(index, widget, data, model)
        ...
        if data.m_type == 1 then
            ...
            local buff = ""
            local strNum = string.format(LanguageConfig.language_Union_24, big)
            buff = strNum.."  "..ConfigHandler:getCheckPointNameOfID(big)
            local str = BiographyData:checkFbState({fbType = type, fbId = big, gkId = Small})
            if str == false then
                buff = strNum.."  "..buff..LanguageConfig.language_Compound_2
            end
