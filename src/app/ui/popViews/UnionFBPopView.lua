...
function UnionFBPopView:onInitUI()
	...
	self._Image_1_t = self.csbNode:getChildByName("Panel_fb_2"):getChildByName("ListView_fb"):getChildByName("model"):getChildByName("Panel_list_fb"):getChildByName("Button_zhan_li_pin"):getChildByName("Image_1")
...
function UnionFBPopView:showFB()
    ...
    local listHandler = function(index, widget, data, model)
        ...
        local onClick = function(event)
            ...
            if data.purchased > 0 then
                ...
                elseif data.allpassed == 0 then
                    ...
                    local ppp = data.m_id
                    local str = BiographyData:checkFbState({fbType = CombatCenter.CombatType.RB_ElitePVE, fbId = data.m_id, gkId = 10})
                    if str then
                        --跳转到团队副本
        ...
        local onZhanLiPin = function(event)
            local str = BiographyData:checkFbState({fbType = CombatCenter.CombatType.RB_ElitePVE, fbId = data.m_id, gkId = 10})
            if str then
                --跳转到团队副本
                Functions.jumpFbSeleckOfData({fbType = CombatCenter.CombatType.RB_Tandui, fbId = data.m_id, isOpenAward = true})
            else
                PromptManager:openTipPrompt(LanguageConfig.language_Union_29)
            end
        end
        ban:getChildByName("Button_zhan_li_pin"):onTouch(Functions.createClickListener(onZhanLiPin, "zoom"))
