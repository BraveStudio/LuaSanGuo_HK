...
function CombatBeforePopView:onInitUI()
	...
	self._zxkz_eh_t = self.csbNode:getChildByName("beforPanel"):getChildByName("zxkz_eh")
	self._zxkz_he_t = self.csbNode:getChildByName("beforPanel"):getChildByName("zxkz_he")
...
function CombatBeforePopView:updateZxkz()
    local heroZxId = ConfigHandler:getHeroZxIdOfId(self.heroInfo.heros[1])
    local heroKzZxId = ConfigHandler:getStrikeZxOfId(heroZxId)
    local enemyZxId, enenmyKzZxId
    if self.enemyInfo.heros then
        enemyZxId = ConfigHandler:getHeroZxIdOfId(self.enemyInfo.heros[1])
        enenmyKzZxId = ConfigHandler:getStrikeZxOfId(enemyZxId)
    else
        enemyZxId = 0
        enenmyKzZxId = 0
    end

    if heroKzZxId == enemyZxId then
        self._zxkz_he_t:setVisible(true)
        self._zxkz_eh_t:setVisible(false)
    elseif enenmyKzZxId == heroZxId then
        self._zxkz_he_t:setVisible(false)
        self._zxkz_eh_t:setVisible(true)
    else
        self._zxkz_eh_t:setVisible(false)
        self._zxkz_he_t:setVisible(false)
    end
