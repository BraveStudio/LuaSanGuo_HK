...
function RewardTipsPopView:onInitUI()
	...
	self._heroSoulView_t = self.csbNode:getChildByName("Panel_2"):getChildByName("heroSoulView")
...
function RewardTipsPopView:onDisplayView()
    ...
    if self.showType ~= nil then
        ...
        elseif self.showType == NoticeManager.REWARD_HERO_SOUL_TIPS then
            self._congration_t:setVisible(true)     
            Functions.loadImageWithSprite(self._congration_t, "tyj/uiFonts_res/congration.png")
            self._heroSoulView_t:setVisible(true)
    ...
    if self.heroSoulMark then 
        Functions.getHeroSoulNodeOfMark(self._heroSoulView_t,self.heroSoulMark)
        local nameNode = self._heroSoulView_t:getChildByName("heroSoulPanel"):getChildByName("name")
        local heroSoulId = HeroSoulData:getHeroSoulInf(self.heroSoulMark).id
        name = ConfigHandler:getHeroSoulNameOfId(heroSoulId)
        ...
        nameNode:setColor(colorValue[quality])    
    end
...
function RewardTipsPopView:setHeroSoulNode( heroSoulMark )
    self.heroSoulMark = heroSoulMark
    assert(heroSoulMark and heroSoulMark > 0,"heroSoulID is error")
end
