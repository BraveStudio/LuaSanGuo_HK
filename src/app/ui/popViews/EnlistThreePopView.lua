...
function EnlistThreePopView:onInitUI()
	...
	self._Text_bao_money_1_t = self.csbNode:getChildByName("Panel_Three_page"):getChildByName("Panel_two_2"):getChildByName("Text_bao_money_1")
	...
	self._Text_bao_money_2_t = self.csbNode:getChildByName("Panel_Three_page"):getChildByName("Panel_three_3"):getChildByName("Text_bao_money_2")
	...
	self._Button_fei_xiang_1_t = self.csbNode:getChildByName("Panel_Three_page"):getChildByName("Panel_two_2"):getChildByName("Button_fei_xiang_1")
	self._Button_fei_xiang_1_t:onTouch(Functions.createClickListener(handler(self, self.onButton_fei_xiang_1Click), "zoom"))
	...
	self._Button_fei_xiang_2_t = self.csbNode:getChildByName("Panel_Three_page"):getChildByName("Panel_three_3"):getChildByName("Button_fei_xiang_2")
	self._Button_fei_xiang_2_t:onTouch(Functions.createClickListener(handler(self, self.onButton_fei_xiang_2Click), "zoom"))
end
...
function EnlistThreePopView:onButton_again_2Click()
    ...
    if self.jumpData ~= nil and self.jumpData.type == 10 then
    ...
    if EnlistData.m_CardType == 4 then
        if self.cardCount == 1 then
            if PropData:getPropNumOfId(79) < g_SampleNewCfg.zml then
                --弹出报错信息
                PromptManager:openTipPrompt(LanguageConfig.language_Enlist_12)
                return false
            end

        else
            if PropData:getPropNumOfId(79) < g_SampleNewCfg.TenZml then
                --弹出报错信息
                PromptManager:openTipPrompt(LanguageConfig.language_Enlist_12)
                return false
            end
...
function EnlistThreePopView:onButton_again_1Click()
    ...
    if G_CurrentLanguage == "hk" then
    	self._Button_fei_xiang_1_t:setVisible(false)
    else
        self._Button_fei_xiang_1_t:setVisible(false)
    end
    self._Image_bao_money_1_t:setVisible(false)
    self._Text_bao_money_1_t:setVisible(false)
...
--@auto code Button_fei_xiang_1 btFunc
function EnlistThreePopView:onButton_fei_xiang_1Click()
    Functions.printInfo(self.debug,"Button_fei_xiang_1 button is click!")
    Functions.sharePhotoToFacebook()
end
--@auto code Button_fei_xiang_1 btFunc end

--@auto code Button_fei_xiang_2 btFunc
function EnlistThreePopView:onButton_fei_xiang_2Click()
    Functions.printInfo(self.debug,"Button_fei_xiang_2 button is click!")
    Functions.sharePhotoToFacebook()
end
--@auto code Button_fei_xiang_2 btFunc end
...
function EnlistThreePopView:showHeroHead()
    ...
    local heroData = {}
    if self.jumpData ~= nil and self.jumpData.type == 10 then 
        heroData = self.jumpData.heroData
    else
        heroData = EnlistData:getEnlistData()
    end
    ...
    for k, v in pairs(heroData) do
        ...
        if self.jumpData ~= nil then
            if self.jumpData.type == 10 then 
                Functions.getHeroHead(self[str],{mark = v.slot})
            else
                Functions.getHeroHead(self[str],{mark = self.jumpData.slot})
            end
            heroName:setText(ConfigHandler:getHeroNameOfId(heroData[k].id))
        else
        ...
        if self.jumpData ~= nil then
            if self.jumpData.type == 10 then 
                if self.cardCount > 0 then
                    Functions.getHeroCrad(self._HeroCard_t, {mark = self.jumpData.heroData[self.cardCount].slot})
                end
            else
...
function EnlistThreePopView:showStar()
    ...
    local onActionbaiStar = function()
        ...
        if self.jumpData ~= nil and (self.jumpData.type == 1 or self.jumpData.type == 5) then
            --self.jumpData = nil
            if self.jumpData.type == 1 then
                ...
                self._Text_bao_money_1_t:setVisible(true)
                self._Button_again_1_t:setVisible(true)
                if G_CurrentLanguage == "hk" then
                    self._Button_fei_xiang_1_t:setVisible(true)
                else
                    self._Button_fei_xiang_1_t:setVisible(false)
                end
            elseif self.jumpData.type == 5 then
                ...
                --self._Image_bao_money_1_t:setVisible(false)
                --self._Button_again_1_t:setVisible(false)
            elseif self.jumpData.type == 10 then 
                self._Text_bao_money_1_t:setString(ActivityData.eventAttr.gold)
                self._Panel_two_2_t:setVisible(true)
                self._Button_back_1_t:setVisible(true)
                self._Panel_see_t:setVisible(true)
...
--        self._Text_bao_money_1_t:ignoreContentAdaptWithSize(true)
       -- self._Image_bao_money_1_t:ignoreContentAdaptWithSize(true)
        if  self.jumpData ~= nil and self.jumpData.type == 10 then
            Functions.initLabelOfString( self._Text_bao_money_1_t, tostring(ActivityData.eventAttr.gold))
            Functions.loadImageWithWidget(self._Image_bao_money_1_t, "commonUI/res/image/yuanbao.png")
        elseif EnlistData.m_CardType == 3 or EnlistData.m_CardType == 1 then
            self._Image_bao_money_1_t:ignoreContentAdaptWithSize(true)
            self._Image_bao_money_1_t:setScale(0.9)
    	...
    	elseif EnlistData.m_CardType == 4 or EnlistData.m_CardType == 2 then
            self._Image_bao_money_1_t:ignoreContentAdaptWithSize(true)
            self._Image_bao_money_1_t:setScale(0.4)
            local str = tostring(g_SampleNewCfg.zml).."/"..tostring(PropData:getPropNumOfId(79))
            Functions.initLabelOfString( self._Text_bao_money_1_t, str)
            Functions.loadImageWithWidget(self._Image_bao_money_1_t, "property_63.png")
        end

    elseif EnlistData.m_CardNum == 10 then
--        self._Text_bao_money_2_t:ignoreContentAdaptWithSize(true)
        -- self._Image_bao_money_2_t:ignoreContentAdaptWithSize(true)
        if self.jumpData == nil and EnlistData.m_CardType == 3  then
            self._Image_bao_money_2_t:ignoreContentAdaptWithSize(true)
            self._Image_bao_money_2_t:setScale(0.9)
        ...
        elseif self.jumpData == nil and EnlistData.m_CardType == 4 then
            self._Image_bao_money_2_t:ignoreContentAdaptWithSize(true)
            self._Image_bao_money_2_t:setScale(0.4)
            local strCount = tostring(g_SampleNewCfg.TenZml).."/"..tostring(PropData:getPropNumOfId(79))
            Functions.initLabelOfString( self._Text_bao_money_2_t, strCount)
            Functions.loadImageWithWidget(self._Image_bao_money_2_t, "property_63.png")
        elseif self.jumpData ~= nil and self.jumpData.type == 10 then 
            self._Image_bao_money_2_t:ignoreContentAdaptWithSize(true)
            Functions.initLabelOfString( self._Text_bao_money_2_t, tostring(ActivityData.eventAttr.gold * 9))
            Functions.loadImageWithWidget(self._Image_bao_money_2_t, "commonUI/res/image/yuanbao.png")
        end
    end
end
