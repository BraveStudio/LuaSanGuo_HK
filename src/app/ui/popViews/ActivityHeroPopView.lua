...
function ActivityHeroPopView:onInitUI()
	...
	self._goldPanel_t = self.csbNode:getChildByName("Panel_140"):getChildByName("bg"):getChildByName("goldPanel")
	self._gold_t = self.csbNode:getChildByName("Panel_140"):getChildByName("bg"):getChildByName("goldPanel"):getChildByName("gold")
	self._freeText_t = self.csbNode:getChildByName("Panel_140"):getChildByName("bg"):getChildByName("freeText")
    ...
    --label list
    self._Text_1_t = self.csbNode:getChildByName("Panel_140"):getChildByName("bg"):getChildByName("Text_1")
	self._Text_1_t:setString(LanguageConfig.ui_activityHero_7)
    --button list
...
function ActivityHeroPopView:onFreebtClick()
    ...
    if ActivityData.eventAttr.freeHeroRemainTime < 1 then
    	ActivityData:RequestTakeCard(1,self.xsHeroType,handler1)
    else
    	ActivityData:RequestTakeCard(2,self.xsHeroType,handler2)
    end
end
...
function ActivityHeroPopView:onPaybtClick()
    ...
    local handler = function(event)
        ...
        self:getController():openChildView("app.ui.popViews.EnlistThreePopView", {isRemove = true,data = {heroData = event.tmp, type = 10,handler = function()
            self:onPaybtClick()
        end}})   
    end
    ActivityData:RequestTakeCard(3,self.xsHeroType,handler)
end
...
function ActivityHeroPopView:getCardWithPay( )
	local handler = function(event)
    	-- self:rankPlayer(self._rankListView_t,event.rankPlayer)	
    	Functions.setAdbrixTag("retension","newhero_buy_nonfree", tostring(PlayerData.eventAttr.m_level))
...
function ActivityHeroPopView:initDisplay()
	...
	self:showHero()
	--元宝绑定显示
	self._gold_t:setString(tostring(ActivityData.eventAttr.gold * 9))
	self._goldPanel_t:getChildByName("gold"):setString(tostring(ActivityData.eventAttr.gold))
...
function ActivityHeroPopView:setFreeOrPayState(state)
	if state then 
		self._freeText_t:setVisible(true)
		self._goldPanel_t:setVisible(false)
	else
		self._freeText_t:setVisible(false)
		self._goldPanel_t:setVisible(true)
	end
end
--显示倒记时
