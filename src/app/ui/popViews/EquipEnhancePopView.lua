...
function EquipEnhancePopView:selectEhanceType(type)
	...
	if type == 1 then --强化
        ...
        local moneyNode = self._qiangHuaPanel_t:getChildByName("needMoney")
		local needMoneyData = ConfigHandler:getQiangHuaRes(equipData.m_id)[2]
		moneyNode:getChildByName("prizeCntLabel"):setString(tostring(needMoneyData[3]))
		local onTargetClick = function( )
			PromptManager:openInfPrompt({type = needMoneyData[2],id = needMoneyData[1],target = moneyNode})
		end
		Functions.setEnabledWidget(moneyNode, true)
		moneyNode:onTouch(Functions.createClickListener(handler(moneyNode, onTargetClick), ""))
		--是否显示所需装备
		...
		local qiangHuaBtHandler = function()
			local handler = function( event )
				...
				if returnData.isSuccess then 
					...
					self.jumpData.handler(self.jumpData.mark)
	...
	elseif type == 3 then --洗练
		...
		self:showNeedPropofEquipId(needProp,equipData.m_id,type)
		local moneyNode = self._xiLianPanel_t:getChildByName("needMoney")
...
function EquipEnhancePopView:showNeedPropofEquipId(target,equipId,type)
	...
	Functions.setEnabledWidget(target, true)
