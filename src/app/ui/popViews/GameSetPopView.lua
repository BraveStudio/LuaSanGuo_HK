...
function GameSetPopView:onInitUI()
	...
	self._guojiaTx_t = self.csbNode:getChildByName("Panel_1"):getChildByName("guojiaTx")
...
function GameSetPopView:onQuitcountrybtClick()
    ...
    NoticeManager:openTips(self:getController(), { handler = function()
        WarData:sendExit()
    end, title = LanguageConfig.ui_gameSetView_2})
end
...
function GameSetPopView:onDisplayView()
    ...
    if G_CurrentLanguage == "ch" then
        ...
        Functions.callAnySdkFuc(function( )
            ...
            if not PluginChannel:isSupportLogout() then 
    ...
    local initCountryName = function(id)
        self._guojiaTx_t:setString(Functions.getCountryNameById(id))
        if id == 0 then
    ...
    self._guojiaTx_t:setString(Functions.getCountryNameById(PlayerData.eventAttr.m_camp))
    Functions.bindUiWithModelAttr(self._guojiaTx_t, PlayerData, "m_camp", function(event)
