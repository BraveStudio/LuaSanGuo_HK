...
function GvgBidRankPopView:onInitUI()
	...
	self._Text_jingjia_LV_t = self.csbNode:getChildByName("Panel_bid_2"):getChildByName("Text_jingjia_LV")
...
function GvgBidRankPopView:onDisplayView()
    ...
    local jinjiaLv = string.format(LanguageConfig.language_GuildBattle_10, GuildBattleData:getBuildingInfo()[data.id].needlevel)
    Functions.initLabelOfString( self._Text_title_t, str, self._Text_zhan_ling_t, LanguageConfig.language_GuildBattle_9, self._Text_jingjia_LV_t, jinjiaLv)
    self:showRank()
    --接收竟价公会排名广播
