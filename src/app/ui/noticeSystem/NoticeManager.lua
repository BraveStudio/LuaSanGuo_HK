...
NoticeManager.REWARD_HERO_SOUL_TIPS = 9
...
--获取后台系统提示数据
--@param: param : { type = , cb = } type:为提示类型，cb:获取成功回调，参数为获取的数据
function NoticeManager:getSystemNoticeData(controller, param)
    assert(param and param.type, "param is error")
...
function NoticeManager:openTips(controller, param)
    ...
    if param.isNotPopAction then
        tips:setIsPopAction(false)
    end
...
function NoticeManager:openRewardTips(controller, param)
  assert(param and param.type, "param is error")
    ...
    elseif param.type == NoticeManager.REWARD_HERO_SOUL_TIPS then
        tips:setShowType(NoticeManager.REWARD_HERO_SOUL_TIPS)
        tips:setHeroSoulNode(param.data.heroSoulMark)
        if param.data.againHandler ~= nil then 
            tips:setAgainHandler(param.data.againHandler)
            tips:setPropData(param.data.propData)
        end
