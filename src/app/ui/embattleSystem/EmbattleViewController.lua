...
function EmbattleViewController:onSavebtClick()
    ...
    -- local anctionElement = UIActionTool:createScaleByAction({sx = 0.5,sy = 0.5, dur = 0.5})
    -- Functions.playSequenceAction(self._infView_t,{anctionElement},0 ,1)
end
...
function EmbattleViewController:onInfbtClick()
    ...
    -- -- UIActionTool:createFadeInAction({actionList = actionList,dur = 0.8})
    -- UIActionTool:createMoveByAction({actionList = actionList,x = 200,y = 0, dur = 0.5})

    -- local tempAction = UIActionTool:createMoveByAction({dir = "RIGHT",dis = 200, dur = 0.5})
    -- UIActionTool:createScaleByAction({actionList = actionList,sx = 0.8,sy = 0.8, dur = 0.8,})
    -- UIActionTool:createRotateToAction({actionList = actionList,dstAngle = 270, dur = 0.8})
    -- UIActionTool:createRotateByAction({actionList = actionList,dstAngle = 90, dur = 0.8})
    -- UIActionTool:createSkewToAction({actionList = actionList,sx = 0,sy = 50, dur = 0.8})
    -- UIActionTool:createSkewByAction({actionList = actionList,sx = 0,sy = -50, dur = 0.8})
    -- UIActionTool:createJumpByAction({actionList = actionList,pos = {x=300,y=300},height = 60,jumps = 5, dur = 0.8})
    -- UIActionTool:createDelayTimeAction({actionList = actionList,delayTime = 2})
    -- UIActionTool:createJumpToAction({actionList = actionList,pos = {x=100,y=100},height = 60,jumps = 5, dur = 0.8})
    -- UIActionTool:createBezierByAction({actionList = actionList,ctrPointOne = {x=200,y=150},
    --     ctrPointTwo = {x=200,y=170},endPoint = {x=440,y=100}, dur = 0.8})
    -- local bezierAction = UIActionTool:createBezierByAction({ctrPointOne = {x=200,y=150},
    --     ctrPointTwo = {x=200,y=170},endPoint = {x=440,y=100}, dur = 0.8})
    -- UIActionTool:createReverseAction({actionList = actionList,actionElement = bezierAction})
    -- -- UIActionTool:createBezierToAction({actionList = actionList,ctrPointOne = {x=200,y=150},
    -- --     ctrPointTwo = {x=200,y=170},endPoint = {x=0,y=0}, dur = 0.8})
    -- self._infView_t:setOpacity(10)
    -- UIActionTool:createTintToAction({actionList = actionList,rgb = {r = 0, g = 255, b = 0}, dur = 0.8})
    -- UIActionTool:createTintByAction({actionList = actionList,rgb = {r = 255, g = 0, b = 0}, dur = 0.5})
    -- UIActionTool:createBlinksAction({actionList = actionList,blinks = 2,dur = 0.8})

    -- UIActionTool:createFadeOutAction({actionList = actionList,dur = 0.8})
    -- Functions.playSequenceAction(self._infView_t,actionList,0,1)
    -- local followAction = UIActionTool:createFollowAction({followedNode = self._infView_t,rect = {x = 0, y = 0, width = 960, height = 480}})
    -- local nodeGrid = cc.NodeGrid:create()
    -- self._infView_t:addChild(nodeGrid)
    -- local sprite = cc.Sprite:create()
    -- Functions.loadImageWithSprite(sprite, "heroHead/50001_h.png")
    -- nodeGrid:addChild(sprite)
    -- Functions.playSequenceAction(self._infView_t,actionList,0,1)
    -- local temp = UIActionTool:createShaky3DAction({dur = 4,gridSize = cc.size(15,10),range = 15,isShakeZ = false})
    -- nodeGrid:runAction(temp)
    -- Functions.playSequenceAction(nodeGrid,temp,0,1)

     -- UIActionTool:playMoveActionWithEase({node = self._infView_t,startPos = cc.p(0,0),
     --    endPos = cc.p(400,400),easeType = EaseActionType.EaseElasticIn,dur = 0.8,handler = function( )
     --    PromptManager:openTipPrompt("finish")
     -- end})
     -- UIActionTool:playSkewActionWithEase({node = self._infView_t,startSkew = {x = 0,y = 0},
     --    endSkew = {x = 0,y = 90} ,easeType = EaseActionType.EaseElasticIn,dur = 3,handler = function( )
     --    PromptManager:openTipPrompt("finish")
     -- end})

     -- UIActionTool:playEaseActionforNode(self._infView_t,startPos,endPos,easeType,dur)
end
