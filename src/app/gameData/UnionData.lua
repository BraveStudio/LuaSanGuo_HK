...
--公会等级的监听
UnionData.UNION_LV_EVENT = "UNION_LV_EVENT"

--公会FB的监听
UnionData.UNION_FB_EVENT = "UNION_FB_EVENT"

--公会购买次数的监听
UnionData.UNION_BUY_EVENT = "UNION_BUY_EVENT"
    ...
    local onUnionCreat = function(event)
        if event.reqtype == 35 then
            ...
            --todayActif    今日捐献资金
                ...
                --公会贡献总数量在7-1，类型为3的接口里同步了一次，所以在捐献时，这里也要保持数据同步
                local UnionInfo = UnionData:getUnionInfoData()
                UnionInfo[1].eventAttr.allActif = data.guildActif
    ...
    local onUnionCreat = function(event)
        if event.reqtype == 36 then
            if event.ret == 1 then
                local data = event.data
                if data.actif then
                    PlayerData.eventAttr.m_actif = data.actif
                end
...
--count     公会商店还可以购买物品的次数
--items     公会商店出售的物品
--公会商店
function UnionData:sendShopInfo(listener)
    local onUnionCreat = function(event)
        if event.reqtype == 39 then
            if event.ret == 1 then
                ...
                for k,v in pairs(event.data.items) do
                    ...
                    info.m_Needlevel = v.needlevel      --需要的等级
                    info.m_maxCount = v.maxCount        --总次数
                    info.m_curCount = v.curCount        --已购买次数
                    info.m_flag = v.flag                --物品标记
                    
                    self.shopInfo[#self.shopInfo + 1] = info
                end
                listener()
            else
