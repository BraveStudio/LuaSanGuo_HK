        ...
        view.showView = function()
            ...
            if popAction then
                transition.execute(view, popAction)
                 self.blackColorLayers_t[view]:setOpacity(0)
                local fadeInAction = UIActionTool:createFadeInAction({dur = 0.6})
                local easeRateAction = UIActionTool:createEaseRateActionAction({actionElement = fadeInAction,rate = 10})
                Functions.playSequenceAction(self.blackColorLayers_t[view], {easeRateAction}, 0,1)
            end
        end
