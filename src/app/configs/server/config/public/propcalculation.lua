...
--@param : param : { heroInfo = , partHeros = , equipInfos = ,heroInfos = }
--@ carInfo (当前卡牌信息); partHeros (偏将信息数组); equipInfos (装备信息数组); heroInfos (英雄信息数组);
...
function _getHerosHandler(param)
	local heros = {}
	if param.heroInfos then
    	for k, v in pairs(param.heroInfos) do
    		if v.id ~= param.heroInfo.id then
    			heros[#heros + 1] = v
    		end
    	end
    end

    if param.partHeros then
    	for k, v in pairs(param.partHeros) do
    		heros[#heros + 1] = v
    	end
    end
...
function pm_GetCardHp(param)
	...
	yfAddPercent = cs_GetYuanFenValueOfType(param, CombatAttrType.hp)
...
function pm_GetCardAttack(param)
	...
	yfAddPercent = cs_GetYuanFenValueOfType(param, CombatAttrType.attack)
...
function pm_GetCardFas(param)
	...
	yfAddPercent = cs_GetYuanFenValueOfType(param, CombatAttrType.fas)
...
function pm_GetCardFaf(param)
	...
	yfAddPercent = cs_GetYuanFenValueOfType(param, CombatAttrType.faf)
...
-- 属性类型 1 血量 2 攻击 3 法术 4 法防
--根据类型获取缘分加成值
function cs_GetYuanFenValueOfType(param, typeValue)
	local yfAddPercent = 0
	local otherHeros = _getHerosHandler(param)
	if otherHeros and #otherHeros > 0 then
		local yfv1,yfv2,yft1,yft2 = cs_GetYuanFenIds(param.heroInfo.id, otherHeros)
		if yft1 == typeValue then
			yfAddPercent = yfAddPercent + yfv1
		end
		
		if yft2 == typeValue then
